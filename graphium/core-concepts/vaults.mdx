...
A vault is the central organizing unit in Graphium AI. Think of it as a project workspace -- a single container that holds your data tables, AI assistants, playbooks, and connections to external systems, all scoped to a specific purpose.
...
**Card View** (the default) displays vaults as cards in a grid layout. Each card shows the vault's color-coded icon, name, description, record count, table count, and last updated date. This view is good for scanning when you have a handful of vaults and want to see descriptions at a glance.
...
Toggle between the two views using the view switcher in the top-right area of the listing page. You can also use the **Show inactive vaults** toggle to include vaults that have been deactivated.

## Creating a Vault

To create a new vault, click the **Create a Vault** card on the workspace home page. Build your tables, schemas, playbooks, and connections from the ground up -- shaping the vault to fit your use case, whether you are tracking cases, managing schedules, or monitoring compliance.

## The Vault Detail View

Once you open a vault, you land in the vault home. The left side of the screen shows a sidebar with the vault's navigation sections:

**Home** -- The vault's main page. Opens the AI chat interface where you can ask questions, get reports, and interact with your configured AI assistants. Recent conversations and available assistants are shown here.
...
**Connections** -- Links to external systems (EMR, spreadsheets, file servers, etc.), with a count badge. This is where you manage your connections and any other external integrations.

**Assistants** -- The list of AI assistants configured for this vault. This is where Admins and Builders create, edit, publish, and archive assistants.

**Playbooks** -- Automated AI workflows configured to run on demand or on a schedule. This is where you create, edit, and monitor your playbooks and their run history.
...
This is where you control who can do what inside the vault. Graphium AI uses role-based access with four levels:

- **Admin** -- Full access to manage the vault, its users, and all data. Can change settings, invite users, and delete the vault.
- **Builder** -- Can create and modify tables and playbooks. Cannot manage users or delete the vault.
- **Contributor** -- Can add and edit data, but cannot modify the vault's structure (schemas, playbooks, connections).
- **Viewer** -- Read-only access. Can browse tables and view data, but cannot change anything.

You invite users by email and assign them a role. Until they accept, they show a "Pending" status in the permissions list. You can reassign roles or deactivate users at any time.
...
Each vault gets its own connections (Community Surgery Center connects to its own Epic instance; Memorial Hospital connects to a different one), its own playbooks (case review alerts tuned to each site's volume), and its own access permissions (site managers are Admins for their own vault, Viewers for others).
