...
Tables are the structured data layer inside a vault. Each table represents a specific type of information -- Cases, Procedures, Providers, Surgeons -- with a defined schema that describes what properties (fields) exist and how they are organized.

If you have worked with databases, the concept is familiar: rows are records, columns are properties (fields). But Graphium AI tables have one important difference -- their schemas are hierarchical. *Child* properties (sub-fields) can be nested inside *parent* Object properties, creating a JSON-like structure where a top-level Object property called "Patient Demographics" can contain *child* properties (sub-fields) like Date of Birth, Gender, and Insurance. This lets you model real-world data relationships without flattening everything into a single row of dozens of columns.
...
The Schema tab shows the table's structure -- the list of properties (fields) that define what data the table holds. Each property (field) is displayed with three attributes:
...
The schema view is hierarchical. Object properties can be expanded to reveal their child properties (fields), showing the nesting structure visually. A search bar at the top ("Search properties...") lets you filter the property (field) list when you have a large schema.
...
Graphium AI supports a range of property (field) types to model different kinds of data. Here is each type with a practical example from healthcare practice management.
...
A *parent* Object property acts as a container that groups related *child* properties (sub-fields) together, creating the hierarchical nesting that makes Graphium AI schemas more expressive than flat spreadsheets. A *parent* Object property does not hold data itself — it organizes related *child* properties together.
...
A calculated field whose value is derived from other properties in the same record using a defined formula. Formula properties update automatically as the underlying data changes.

- **Total Duration** -- Calculated as the sum of Procedure Duration values across related records
- **Age** -- Derived from Date of Birth relative to today's date
...
When adding fields, you pick a name, a key (the snake_case identifier), and a type. You can nest properties (fields) inside Object properties to create hierarchy. The schema editor lets you reorder fields and add new ones at any level of the hierarchy.

You can also search your existing properties (fields) using the "Search properties..." bar at the top of the Schema tab -- useful when your table has dozens of fields and you need to find or verify a specific one.
...
Scanning the property list, they see top-level properties (fields) like Case ID (Auto ID), Case Date (Date), and Procedure Duration (Number). Then they notice an Object property called Patient Demographics. Expanding it, they find Date of Birth (Date), Gender (Text), and Insurance (Text) nested inside. This tells them that patient demographic data is not scattered as loose columns -- it is logically grouped, which will matter when they configure the AI assistant to pull compliance reports filtered by insurance type.
