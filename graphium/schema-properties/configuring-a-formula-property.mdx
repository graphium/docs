...
Formula properties are useful for combining existing field values into a derived output — for example, concatenating a company name and invoice number into a formatted label, or building a summary from multiple fields.
...
Template mode is the available expression mode. It uses `{{internal_name}}` placeholders to insert property values into a text string.
...
Type your expression into the text area. The expression field is required — a validation error is shown if you attempt to save with an empty expression.
...
|---|---|
| Add or edit a Formula property on a vault table | Builder or Admin |
