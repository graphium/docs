...
If you have worked with databases, the concept is familiar: rows are records, columns are properties (fields). But Graphium tables have one important difference -- their schemas are hierarchical. Properties (fields) can be nested inside Object properties, creating a JSON-like structure where a top-level Object called "Patient Demographics" can contain child properties (fields) like Date of Birth, Gender, and Insurance. This lets you model real-world data relationships without flattening everything into a single row of dozens of columns.
...
An Object property acts as a container that groups related child properties (fields) together, creating the hierarchical nesting that makes Graphium schemas more expressive than flat spreadsheets. An Object property does not hold data itself -- it organizes related properties together.
